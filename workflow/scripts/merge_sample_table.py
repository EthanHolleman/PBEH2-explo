...
def main():
    ...
    sample_df['sample_id'] = sample_df.apply(
        lambda row: extract_sample_id_table(row),
        axis=1
    )
...
def extract_sample_id_table(sample_table_row):

    return int(sample_table_row['Sample_ID'].split('-')[-1])
...
def extract_sample_id(composite_call_row):
    ...
    """
    print(composite_call_row)
    return int(composite_call_row['sample'].split('_')[1].split('bcBC')[-1])
